...
public class AnalogPrototypeInspector {
	
	private Rational T;
	
	public AnalogPrototypeInspector( AnalogPrototype proto ) {
		T = proto.getTransferFunction();
	}
	
	
	public double getMagnitude( double omega ) {
		return T.evaluate( new Complex( 0.0, omega ) ).abs();
	}
