...
public class EnvelopeSacCommandTest {
    ...
    public void testEnvelope() {
        ...
        float tolerance = 0.0065f;

        TestUtil.runCommand("envelope", new EnvelopeSacCommand());

        List<SacTraceData> lstd = SacDataModel.getInstance().getData();
        SacTraceData test = lstd.get(0);

        SacTraceData truth = TestUtil.getTruth("envelopeTruth.sac", TEST_FILE_DIRECTORY);
        TestUtil.compareTwoSacfileArraysExact(truth, test, "testEnvelope", tolerance);
    }
}
