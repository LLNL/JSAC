...
public class HanningSacCommandTest {
    ...
    public void testHanning() {
        ...
        TestUtil.runCommand("hanning", new HanningSacCommand());

        List<SacTraceData> lstd = SacDataModel.getInstance().getData();
        SacTraceData test = lstd.get(0);

        SacTraceData truth = TestUtil.getTruth("hanningTruth.sac", TEST_FILE_DIRECTORY);
        //        TestUtil.compareTwoSacfileArraysExact(truth, test, "testHanning", tolerance);
    }
}
